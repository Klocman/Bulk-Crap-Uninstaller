...
zh.depdownload_msg=在继续设置之前，需要安装以下应用程序：%n%n%1%n 立即下载并安装？
...
zh.depinstall_missing=在继续安装之前，必须安装 %1。请安装 %1 并再次运行安装程序。
...
zh.depinstall_error=安装依赖项时出现错误。请重启计算机并再次运行安装程序，或手动安装以下依赖项：%n
