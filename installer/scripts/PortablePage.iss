...
zh.InstallTypeChoiceTitle =安装方式
...
zh.InstallPortableInfo =便携式安装不会在系统中注册，只会将文件解压缩到指定目录。您可以将目录设置为任何您想要的位置，例如 USB 驱动器。您可以自由移动该目录，因为整个程序都在其中。
...
zh.InstallStandardInfo =此选项会将 BCUninstaller 作为普通应用程序安装到电脑上。标准卸载程序将被创建，并在 “程序和功能 ”以及其他第三方卸载程序中可见。
